...
.. http:post:: /token
    ...
    Generates a ``JSON Web Token`` that is used for authentication when making calls to the other endpoints of the API. This token expires after ``1 hour``. Used the ``/token/verify`` to verify the validity of a token.
